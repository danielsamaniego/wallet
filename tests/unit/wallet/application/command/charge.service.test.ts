...
import { ChargeService } from "@/wallet/application/command/charge/service.js";
...
describe("ChargeService", () => {
  ...
  function newMovement(): Movement {
    return Movement.create({ id: "mov-1", type: "charge", platformId: "platform-1", createdAt: 1700000000000 });
  }
  ...
  describe("Given an active user wallet with 10000 cents balance and no holds", () => {
    ...
    describe("When charging 3000 cents", () => {
      ...
      it("Then a Transaction is created with type 'charge' and status 'completed' referencing the given Movement", async () => {
        await sut.execute(ctx, cmd, newMovement());
