...
const mainLogTag = "ChargeService";

/**
 * Business core of a platform-initiated charge. Shared by `ChargeUseCase`
 * (sync path) and the Phase 2 async worker. Must be called inside an open
 * transaction with the per-wallet lock already acquired; the caller is
 * responsible for having persisted `movement`.
 */
export class ChargeService {
  ...
  async execute(ctx: AppContext, cmd: ChargeCommand, movement: Movement): Promise<ChargeResult> {
