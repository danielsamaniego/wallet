...
export function withdrawRoute(deps: MutationHandlerDeps) {
  return handlerFactory.createHandlers(
    describeRoute({
      ...
      responses: {
        ...
        202: {
          description:
            "Async pipeline accepted the withdrawal but the worker did not finish before the wait window. Poll GET /v1/movements/{id} for the terminal state.",
          content: { "application/json": { schema: resolver(ResponseSchema) } },
        },
        400: {
    ...
    async (c) => {
      ...
      if (deps.asyncDispatch) {
        const outcome = await asyncDispatch<WithdrawResult>(
          ctx,
          deps.commandBus,
