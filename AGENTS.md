...
- **System wallets:** Per `(platform, currency)`, with `owner_id = "SYSTEM"` and `is_system = true`. **Sharded** into `N = platforms.system_wallet_shard_count` rows (default 32, only-increase). Shards materialise lazily + idempotently on first wallet creation for that platform/currency via `ensureSystemWalletShards`, called **outside** the SERIALIZABLE tx. Every movement routes to a shard via `systemWalletShardIndex(userWalletId, shardCount)` (pure FNV-1a in `utils/kernel/shard.ts`) and writes with `adjustSystemShardBalance` (atomic `UPDATE … RETURNING`). System wallets cannot be frozen or closed; public balances aggregate via `sumSystemWalletBalance` and shards never appear in the `/v1/wallets` listing. See `docs/architecture/systemPatterns.md` § "System Wallet Sharding".
...
- **Cross-context dependencies:** A BC's domain and application layers must never import types, ports, or implementations from another BC (repositories, aggregates, errors, DTOs). When cross-BC interaction is unavoidable, apply the narrowest pattern that fits — see `docs/architecture/backend-architecture.md` § "Cross-Context Dependencies" for the full decision guide. Short version: (A) consumer defines a narrow port in its own `domain/ports/`, adapter in its own `infrastructure/adapters/outbound/` delegates to the provider — use this by default; (B) promote to `common/` if the feature is cross-cutting and used by multiple BCs; (C) extract to a new BC only if the coupling reveals a misplaced domain concept. Wiring files (`*.module.ts`, `wiring.ts`) are exempt — composition is their job.
- **CQRS file structure:** Each command use case has three files colocated under `application/command/<name>/`: `command.ts` for pure command/result types, `service.ts` for the business-rule core (the deposit/withdraw/etc. domain logic the async worker will reuse — see HIGH_CONCURRENCY_PLAN.md § Phase 2), and `usecase.ts` for the orchestration class (lock + tx envelope + Movement lifecycle + delegate to service). Each query uses `query.ts` for query/DTO types and `usecase.ts` for the use case class. Service responsibilities: validate domain rules, mutate aggregates, write transactions/ledger entries — assumes the caller has already acquired the lock, opened the transaction, and persisted the Movement. Use case responsibilities: open lock + tx, create + save Movement (sync path) or transition pending Movement (Phase 2 async worker path), then call `service.execute(ctx, cmd, movement)`.
...
- **Two-layer concurrency model.** Outer = **distributed lock** via `LockRunner` (`src/utils/application/lock.runner.ts`), a Redis-backed per-resource mutex wrapping the transaction. Inner = **optimistic locking** on `wallet.version` plus Serializable isolation. `TransactionManager` retries the inner layer internally up to 5 times with full-jitter exponential backoff (per-attempt ceilings 30/60/120/240ms). If exhausted, `409 VERSION_CONFLICT`. If the outer layer times out waiting, `409 LOCK_CONTENDED`. Both are transient — client retries with the same idempotency key. Do not use `SELECT FOR UPDATE` in domain ports. All mutations use idempotency keys with the atomic acquire pattern. Database `CHECK` constraints remain the safety net.
- **LockRunner usage:** Mutating use cases wrap their transactional body in `lockRunner.run(ctx, ["<prefix>:<id>"], async () => txManager.run(...))`. Lock FIRST, tx SECOND — never the reverse. Use key prefix `wallet-lock:<walletId>` for wallet mutations. Multi-wallet (transfer) passes both keys; the runner sorts + dedupes to prevent A↔B deadlocks. For commands keyed by a derived resource (e.g. `captureHold` takes a `holdId`), resolve the walletId *and validate platform ownership* outside the lock — acquiring a lock on an unvalidated cross-tenant key is a DoS vector. When Redis is unreachable or the feature is off, the runner falls through transparently and optimistic locking takes over. Mockable in tests via `createMockLockRunner()` from `test/helpers/mocks/`. Canonical guide: `docs/architecture/systemPatterns.md` § "Distributed Lock".
