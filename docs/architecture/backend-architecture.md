...
- **Optimistic locking (inner layer)**: All wallet mutations (single and multi-wallet). On version mismatch, the `TransactionManager` retries internally (15 attempts, full-jitter exponential backoff with per-attempt ceilings 30/60/120/240/480 ms then capped at 500 ms — worst-case ~5.93 s sleep budget) before escalating `409 VERSION_CONFLICT` to the client. Transactions run under **Serializable isolation**; PostgreSQL serialization failures (40001/P2034/`TransactionWriteConflict`) are also retried internally and, if exhausted, mapped to `VERSION_CONFLICT`.
