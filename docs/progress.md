...
- [ ] **Async movement processing — Phase 2B.6+**: (a) worker handler real body — replace the Phase 1C scaffolding so the inbound adapter loads the Movement by id, `markProcessing` (atomic claim), narrows `movement.type` to dispatch the appropriate `<op>Service.execute(ctx, narrowedCmd, movement)`, then `markPosted` / `markFailed` and publishes the terminal status via `RedisResultPublisher`; (b) refactor all 7 mutating HTTP handlers to branch on `WALLET_ASYNC_PROCESSING_ENABLED`: when on, dispatch `EnqueueMovementCommand` + call `resultSubscriber.waitFor(movementId, { timeoutMs: WALLET_HANDLER_WAIT_MS })` for 200 (fast path) or 202 (slow path); when off, keep today's inline dispatch. Single switch, per-platform rollout. Phase 3 rolls out per-platform and per-percentage. See `HIGH_CONCURRENCY_PLAN.md`.
