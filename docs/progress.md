...
- [ ] **Async movement processing — Phase 1D / Phase 2+**: ports `IMovementQueuePublisher` + `IResultPublisher` as interface-only scaffolding (Phase 1D). Phase 2 implements `EnqueueMovementUseCase` and `ProcessMovementUseCase`, wires QStash/Redis adapters, replaces the scaffolded worker handler body with the real dispatch, and refactors handlers to "enqueue + wait-on-pub/sub" behind the `WALLET_ASYNC_PROCESSING_ENABLED` flag. Phase 3 rolls out per-platform and per-percentage. See `HIGH_CONCURRENCY_PLAN.md`.
