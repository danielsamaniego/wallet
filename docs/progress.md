...
- [x] Multi-currency support: explicit currency catalog (USD, EUR, MXN, CLP, KWD, GBP) with `wallets_supported_currency` CHECK constraint in PostgreSQL
...
- [x] **Batch per-operation metadata**: each op in `POST /v1/wallets/batch-operations` may carry its own `metadata`, merged over the batch-level `metadata` (op keys win) onto its transaction — lets a heterogeneous batch (e.g. a settlement whose legs have different consumer reason keys) label each leg while sharing batch keys (correlation id). No schema change. E2E in `tests/e2e/wallet/batch-operations.e2e.test.ts`.
- [x] **Movement breakdown analytics**: per-wallet `GET /v1/wallets/:walletId/analytics/movement-breakdown` + platform-wide `GET /v1/analytics/movement-breakdown` (new `/v1/analytics` group). Credit/debit/net/count buckets grouped by `type`, `day`/`week`/`month`, a generic `metadata` key, or `owner` (platform only); `direction` filter + `owner_id` narrowing. Sums the wallet's own signed ledger entries, **excludes system wallets** (double-entry would net to zero). Agnostic metadata grouping. E2E in `tests/e2e/wallet/movement-breakdown.e2e.test.ts`.
- [x] **Breakdown metadata-value filter + min/max**: both breakdown endpoints accept `metadata_filter_key` + `metadata_filter_value` (validated as a pair) to narrow **before** grouping on a generic JSON key/value, independent of `group_by` (e.g. group by month while filtering one reason key); each bucket also carries `min_minor`/`max_minor` (signed `MIN`/`MAX`). Averages are derivable by the consumer (`sum/count`) and deliberately not stored (no fractional money). No schema change.
- [x] **Platform statement-by-movement**: `GET /v1/statement/:movementId` (new `/v1/statement` group) resolves a movement without its wallet id (scope from the API key). Returns the user-facing faces (system/omnibus excluded): one for a normal movement, two for a transfer (sender debit + receiver credit), each with `wallet_id` + `owner_id`. Anchored on `ledger_entries`; 404 when no user face for the platform. E2E in `tests/e2e/wallet/statement.e2e.test.ts`. No schema change.
- [x] **Statement direction filter + opt-in total**: `GET /v1/wallets/:walletId/statement` accepts `direction=credit|debit` (narrows on the wallet's own ledger-entry side via the `ledgerEntries.some` relation) and `include_total=true` (adds `total` = full match count across all pages via a cursor-free `COUNT`, respecting active filters). E2E in `tests/e2e/wallet/statement.e2e.test.ts`.
