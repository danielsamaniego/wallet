...
- [ ] **Async movement processing — Phase 1C+**: empty worker route `POST /internal/worker/process-movement` with QStash signature verification (Phase 1C), ports `IMovementQueuePublisher` + `IResultPublisher` as interface-only scaffolding (Phase 1D). Phase 2 implements `EnqueueMovementUseCase` and `ProcessMovementUseCase`, wires QStash/Redis adapters, and refactors handlers to "enqueue + wait-on-pub/sub" behind the `WALLET_ASYNC_PROCESSING_ENABLED` flag. Phase 3 rolls out per-platform and per-percentage. See `HIGH_CONCURRENCY_PLAN.md`.
