...
- [ ] **Async movement processing — Phase 2+**: (a) denormalise `Movement.platformId` so async `pending`/`processing` movements (no transactions yet) are still resolvable through `GET /v1/movements/{id}`; (b) implement `EnqueueMovementUseCase` (handler side — INSERT pending + publish + wait-on-Redis) and `ProcessMovementUseCase` (worker side — claim pending, dispatch existing use case, publish result); (c) implement adapters `QStashMovementQueuePublisher` and `RedisResultPublisher`; (d) refactor mutating handlers to flip between inline and enqueue-and-wait via `WALLET_ASYNC_PROCESSING_ENABLED`. Phase 3 rolls out per-platform and per-percentage. See `HIGH_CONCURRENCY_PLAN.md`.
