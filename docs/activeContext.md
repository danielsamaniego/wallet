...
**Phase 2B.6+ (next):**
1. Worker handler real body (replace the Phase 1C scaffolding): load Movement via `findById` (using only `movement_id`; the worker is platform-agnostic), `markProcessing` (atomic claim — short-circuits to 200 noop on second delivery), narrow `movement.type` to dispatch the appropriate `<op>Service.execute(ctx, narrowedCmd, movement)`, then `markPosted` / `markFailed` and publish the terminal status via `RedisResultPublisher`. The service layer holds lock + tx exactly as the sync path does today.
2. HTTP handler refactor: each of the seven mutating handlers branches on `WALLET_ASYNC_PROCESSING_ENABLED`. When on: dispatch `EnqueueMovementCommand` + call `resultSubscriber.waitFor(movementId, { timeoutMs: WALLET_HANDLER_WAIT_MS })` → 200 with full body on fast path, 202 `{ movement_id, status: "processing" }` on slow path. When off: keep today's inline dispatch. Single switch, per-platform rollout.
3. E2E coverage for the async path (forging a signed worker call inside the harness is heavy but tractable for at least the happy + claim-race cases).
