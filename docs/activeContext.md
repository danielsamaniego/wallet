...
**Phase 2B.7+ (next):**
1. HTTP handler refactor: each of the seven mutating handlers branches on `WALLET_ASYNC_PROCESSING_ENABLED`. When on: serialise the operation command as `queue_payload` (BigInt → string, including a pre-resolved `walletId` for `hold_capture`), dispatch `EnqueueMovementCommand`, then call `resultSubscriber.waitFor(movementId, { timeoutMs: WALLET_HANDLER_WAIT_MS })` → 200 with the full body on the fast path, 202 `{ movement_id, status: "processing" }` on the slow path. When off: keep today's inline dispatch. Single switch, per-platform rollout.
2. Wire `RedisResultSubscriber` in `wiring.ts` (reuses the same ioredis client as the publisher) and surface it to the HTTP handlers via `Dependencies`.
