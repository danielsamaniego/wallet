...
**Phase 2B.7b (next):**
1. Build a small `executeMutationAsync` helper at `application/worker/asyncDispatch.ts` (or inline per-handler if it stays simple) that, given the `Dependencies` plus an `EnqueueMovementCommand`, dispatches via the bus and `waitFor`s on the subscriber → returns either the `body` (posted), the failed reason (failed), or `null` (timeout → 202).
2. Refactor the six mutating handlers (`deposit`, `withdraw`, `charge`, `adjustBalance`, `transfer`, `captureHold`) to branch on `config.asyncProcessingEnabled && deps.resultSubscriber`. When async-eligible: serialise the operation command into `queue_payload` (BigInt → string, including a pre-resolved `walletId` for `captureHold` so the worker need not redo the hold→wallet lookup), dispatch the enqueue, await, build the same 200 response shape as the sync path from `body`. When unable to dispatch (bus throws "no handler"): fall back to today's inline dispatch — never deny the request because the operator opted into async before the wiring landed.
3. Update handler tests to cover both branches per endpoint.
4. E2E coverage for the async path (the happy + claim-race cases at minimum — forging a signed worker call inside the harness is heavy but tractable for that scope).
