...
**Lock key environment namespace landed.** New `WALLET_LOCK_KEY_PREFIX` env (e.g. `dev:`, `preview:`) is prepended by `LockRunner` to every distributed-lock key before it reaches the Redis adapter. Isolates environments that share one Redis instance — without it, a non-prod environment seeded with a copy of prod data locks the exact same keys (`wallet-lock:<id>` with identical UUIDs) and blocks prod mutations. Default `""` keeps existing deployments byte-identical. Callers keep passing logical keys; logs show the full namespaced key. Config-validated (max 32 chars, `[A-Za-z0-9:_-]`). No schema change.

**Platform-wide statement-by-movement landed.** New `GET /v1/statement/:movementId` resolves a movement **without knowing its wallet** (scope from the API key) — the global sibling of the per-wallet `GET /v1/wallets/:walletId/statement/:movementId`. By double-entry a movement has two ledger entries; the system/omnibus counterpart is excluded, so it returns the **user-facing faces**: ONE for a normal movement, TWO for a transfer (sender debit + receiver credit), each annotated with `wallet_id` + `owner_id`. Anchored on `ledger_entries` (the per-face grain), `404` when no user-facing face exists for the platform. Lets a consumer inspect any movement by its bare id without a reverse movement→wallet lookup. Full 100% unit coverage + E2E in [tests/e2e/wallet/statement.e2e.test.ts](../tests/e2e/wallet/statement.e2e.test.ts). No schema change.

**Read-side analytics filters & metrics extended.** The statement and both movement-breakdown endpoints gained the narrowing/shaping a consumer needs to reproduce richer movement queries against the Wallet (instead of its own legacy ledger), without leaking consumer concepts into the service. Statement (`GET /v1/wallets/:walletId/statement`): `direction=credit|debit` keeps only the wallet's own ledger-entry side (applied on the `ledgerEntries.some` relation, not a transaction column), and `include_total=true` adds `total` = the full match count across all pages (cursor-free `COUNT`, opt-in so the common path pays nothing; respects all active filters). Movement breakdown (per-wallet + platform): `metadata_filter_key` + `metadata_filter_value` narrow **before** grouping on an arbitrary JSON key (bound as params, never interpolated), independent of `group_by` — so a caller can group by month while filtering on a single metadata value; and each bucket now carries `min_minor`/`max_minor` (signed `MIN`/`MAX`). Averages are intentionally **not** added: they are derivable by the consumer (`avg = sum/count`, `avgAbs = (credits − debits)/count`) and would introduce fractional money into the service. Service stays consumer-agnostic. No schema change → no migration. Full 100% unit coverage; E2E extended in [tests/e2e/wallet/statement.e2e.test.ts](../tests/e2e/wallet/statement.e2e.test.ts) and [tests/e2e/wallet/movement-breakdown.e2e.test.ts](../tests/e2e/wallet/movement-breakdown.e2e.test.ts).

**Movement breakdown analytics landed.** Two read endpoints that aggregate movements into credit/debit/net/count buckets over a time range: per-wallet `GET /v1/wallets/:walletId/analytics/movement-breakdown` (alongside cash-flow) and platform-wide `GET /v1/analytics/movement-breakdown` (new top-level `/v1/analytics` group, scope from the API key). Group by `type`, time bucket (`day`/`week`/`month`), an arbitrary `metadata` key, or (platform only) `owner`; optional `direction` filter and `owner_id` narrowing. Aggregation sums the wallet's **own** signed ledger entries and **excludes system wallets** — by double-entry the omnibus side is the counterpart of every user movement, so including it would net the platform to zero; excluding it yields the real flow to/from users. Grouping by a consumer-supplied `metadata.<key>` keeps the service agnostic (it never learns what a "reason key" is). Full 100% unit coverage; E2E in [tests/e2e/wallet/movement-breakdown.e2e.test.ts](../tests/e2e/wallet/movement-breakdown.e2e.test.ts). No schema change.

**Atomic batch operations landed (`POST /v1/wallets/batch-operations`).** New command `ApplyBatchOperations` applies N operations (`deposit` | `withdraw` | `charge`) atomically — all in one SERIALIZABLE transaction under one idempotency key, all-or-nothing. Built for consumer settlement flows (sale credit + commission + holdback in one call) so partial application is impossible. **Each operation produces its own normal movement** (deposit/withdrawal/charge) — no synthetic grouping type; atomicity comes from the shared transaction + idempotency key, and operations correlate via shared `reference`/`metadata`. Credits apply before debits; the resulting available balance of every touched wallet must stay non-negative or the whole batch is rejected with `INSUFFICIENT_FUNDS`. Multi-wallet locking reuses the transfer pattern (sorted + deduped lock keys). **Each operation may also carry its own `metadata`, merged OVER the batch-level `metadata` (operation keys win)** — so a heterogeneous batch (e.g. a settlement whose legs have different consumer reason keys) can label each resulting transaction individually while still sharing batch keys like a correlation id. Full 100% unit coverage maintained; E2E in [tests/e2e/wallet/batch-operations.e2e.test.ts](../tests/e2e/wallet/batch-operations.e2e.test.ts).
...
- TransactionManager: Serializable isolation + internal retry (15 attempts, full-jitter exponential backoff capped at 500 ms) before escalating VERSION_CONFLICT to client
...
- **Transactions**: Serializable isolation level; TransactionManager retries internally (15 attempts, full-jitter exponential backoff with per-attempt ceilings 30/60/120/240/480 ms then capped at 500 ms — worst-case ~5.93 s sleep budget) for VERSION_CONFLICT and PostgreSQL serialization failures before escalating to client
