...
**Phase 3 (completed — full AppError fidelity across the queue boundary):**
- `MovementResult` gains `failedKind?: string` (the `ErrorKind` enum value — `NOT_FOUND`, `DOMAIN_RULE`, etc.) and `failedCode?: string` (the AppError code — `WALLET_NOT_FOUND`, `INSUFFICIENT_FUNDS`, etc.). Absent when the worker caught a non-AppError throw (raw Prisma errors that escaped translation).
- `ProcessMovementUseCase` catch branch inspects `AppError.is(err)`. When true, captures `err.msg`/`err.kind`/`err.code` and passes all three to `resultPublisher.publish`. Logs include kind+code for observability. `markFailed` uses `AppError.msg` (the raw user-facing message) instead of the cause-decorated `Error.message`.
- `RedisResultPublisher` + `RedisResultSubscriber` round-trip the new fields transparently via JSON (no adapter changes — the loose record passes through).
- `asyncDispatch` helper's `failed` outcome variant gains optional `failedKind` + `failedCode` mirroring the published result. New companion `rebuildAppErrorFromFailedOutcome(outcome)` reads them and: when both are present AND `failedKind` is a recognised `ErrorKind` value, rebuilds with `AppError.create(kind, code, message)`; otherwise falls back to `AppError.domainRule("MOVEMENT_FAILED", reason)`. The defensive fallback covers future enum drift (a worker version publishing an unknown kind cannot crash the handler).
- All six mutating handlers replaced their `throw AppError.domainRule("MOVEMENT_FAILED", outcome.failedReason)` with `throw rebuildAppErrorFromFailedOutcome(outcome)`. The global `onError` then maps the reconstructed AppError to the same HTTP status the sync path would have returned: 404 for `NOT_FOUND`, 422 for `DOMAIN_RULE`, 409 for `CONFLICT`, etc.
- Verified live against the local stack (Postgres + Redis + QStash dev server) with `WALLET_ASYNC_PROCESSING_ENABLED=true`:
  - Async withdraw over balance → **HTTP 422** `{"error":"INSUFFICIENT_FUNDS","message":"wallet ... has insufficient available funds"}` (was a generic `MOVEMENT_FAILED` before this phase).
  - Async deposit on non-existent wallet → **HTTP 404** `{"error":"WALLET_NOT_FOUND","message":"wallet ... not found"}` (was 422 before).
  - Async deposit happy path → **HTTP 201** with `{ transaction_id, movement_id }` in 192ms when QStash dev server is warm (identical to sync).
- The async path is now **drop-in compatible** with the sync path on all four observable response dimensions: status code, error code, message, and body shape (on success). The only remaining client-visible difference is the optional 202 timeout response (which sync never returns).
- Unit: 1140/1140 at 100% coverage. New tests: 4 `rebuildAppErrorFromFailedOutcome` cases (full fidelity round-trip, fallback on missing fields, defensive fallback on unknown kind, partial fidelity), 1 round-trip case each on publisher + subscriber for the new fields, 2 ProcessMovementUseCase AppError cases (DomainRule + NotFound), 1 asyncDispatch helper case forwarding kind/code, 2 handler tests asserting async returns 404 / 422 with original codes. E2E: 277/277 unchanged.

**Phase 3+ remaining (post-Phase-3 polish, none blocking activation):**
